...
const PLATE_PREFIXES = {
  Gauteng: ['GP'], 'Western Cape': ['WC', 'CA'], 'KwaZulu-Natal': ['NP', 'ND'],
...
function generateRoadNetwork(centerLat, centerLng, rand) {
  ...
  angles.forEach((angle, i) => {
    ...
    const wobbleLat = 0.0008;
    const wobbleLng = 0.0008 * Math.cos(centerLat * Math.PI / 180);
    for (let j = 0; j < len; j++) {
      const t = j / (len - 1);
      const dist = t * (5 + rand() * 3);
      points.push([
        centerLat + dist * latScale * Math.cos(rad) + (rand() - 0.5) * wobbleLat,
        centerLng + dist * lngScale * Math.sin(rad) + (rand() - 0.5) * wobbleLng,
      ]);
    }
...
function randomPlate(province, rand) {
  ...
  return `${prefix} ${num} ${letters[Math.floor(rand() * letters.length)]}${letters[Math.floor(rand() * letters.length)]}`;
}
...
function generateVehicles(roadNetworks, multiplier, province, rand) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    vehicles.push({
      ...
      fine: hasFine ? { type: fineType, amount: fineAmounts[Math.floor(rand() * fineAmounts.length)] } : null,
    });
  }
  for (let i = 0; i < Math.floor(2 + multiplier * 2); i++) {
    vehicles.push({
      id: `police-${i + 1}`, plate: `POLICE-${i + 1}`,
      route: routeKeys[Math.floor(rand() * routeKeys.length)],
      direction: rand() > 0.5 ? 1 : -1, speed: 0.00025 + rand() * 0.00020, isPolice: true, fine: null,
    });
  }
  return vehicles;
}
...
function generateActivityFeed(province, cityName, rand) {
  ...
  return ACTIVITY_TEMPLATES.map((tmpl, i) => ({ id: i + 1, ...tmpl(makePlate(), cityName) }));
}
...
const carIcon = (color, plate, hasFine) => new L.DivIcon({
  className: 'custom-marker',
  html: `<div class="relative"><div class="w-7 h-7 rounded-full flex items-center justify-center shadow-lg border-2 border-white" style="background:${color}"><svg viewBox="0 0 24 24" class="w-3.5 h-3.5 stroke-white fill-none" stroke-width="2.5"><rect x="1" y="9" width="22" height="11" rx="2"/><path d="M5 9V6a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v3"/><circle cx="7" cy="17" r="1.5"/><circle cx="17" cy="17" r="1.5"/></svg></div><div class="absolute -top-0.5 -right-3 text-[7px] font-bold text-white bg-black/70 px-1 rounded whitespace-nowrap">${plate}</div>${hasFine ? '<div class="absolute -bottom-1 -right-1 w-3 h-3 bg-red-500 rounded-full border border-white animate-pulse"></div>' : ''}</div>`,
  iconSize: [35, 35], iconAnchor: [17, 17],
});
const policeIcon = new L.DivIcon({
  className: 'custom-marker',
  html: '<div class="w-7 h-7 rounded-full flex items-center justify-center shadow-lg border-2 border-blue-300 animate-pulse" style="background:#1d4ed8"><svg viewBox="0 0 24 24" class="w-3.5 h-3.5 stroke-white fill-none" stroke-width="2.5"><rect x="1" y="9" width="22" height="11" rx="2"/><path d="M5 9V6a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v3"/><circle cx="7" cy="17" r="1.5"/><circle cx="17" cy="17" r="1.5"/></svg></div>',
  iconSize: [28, 28], iconAnchor: [14, 14],
});
const pinIcon = new L.DivIcon({
  className: 'custom-marker',
  html: '<div class="relative"><div class="w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center shadow-lg border-2 border-white animate-bounce"><svg viewBox="0 0 24 24" class="w-3 h-3 stroke-white fill-none" stroke-width="3"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div><div class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-blue-500 rounded-full"></div></div>',
  iconSize: [20, 24], iconAnchor: [10, 24],
});
...
const SmoothMarker = ({ vehicle, routePoints }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (s.progress >= 1) { s.progress = 0; s.segment += s.dir; if (s.segment >= totalSegments) { s.segment = totalSegments - 1; s.dir = -1; } if (s.segment < 0) { s.segment = 0; s.dir = 1; } }
  ...
  return (
    <>
      <Marker ref={markerRef} position={startPos} icon={vehicle.isPolice ? policeIcon : carIcon(vehicle.color, vehicle.plate, !!vehicle.fine)}>
        <Popup><div className="text-xs"><p className="font-bold text-slate-900 mb-1">{vehicle.plate}</p>{vehicle.isPolice && <p className="text-blue-600 font-medium">🚔 Patrol Unit</p>}{vehicle.fine && <div className="mt-1 p-1.5 bg-red-50 rounded"><p className="text-red-600 font-medium">⚠ {vehicle.fine.type.replace('_', ' ')}</p><p className="text-red-500 font-bold">R {vehicle.fine.amount.toLocaleString()}</p></div>}{!vehicle.fine && !vehicle.isPolice && <p className="text-emerald-600 font-medium">✓ Clear</p>}</div></Popup>
      </Marker>
...
const LiveMap = () => {
  ...
  const [activeFilters, setActiveFilters] = useState({
    violations: true, expiredDiscs: true, roadblocks: true, hotspots: true, vehicles: true, revenue: true,
  });
  ...
  const handleMapPick = useCallback((latlng) => {
    ...
    setRoadblocksList(prev => [...prev, { id: Date.now(), lat: latlng[0], lng: latlng[1], name: `${effectiveCity} Roadblock`, officers: 4 }]);
  ...
  const getDotColor = (color) => ({ red: 'bg-red-500', amber: 'bg-amber-500', blue: 'bg-blue-500', emerald: 'bg-emerald-500' }[color] || 'bg-slate-500');

  if (!sim) return <AdminLayout><div className="flex-1 flex items-center justify-center"><p className="text-slate-500 text-sm">No simulation data for this jurisdiction.</p></div></AdminLayout>;

  return (
    <AdminLayout>
      <div className="flex h-full">
        <div className="flex-1 relative">
          <MapContainer center={sim.center} zoom={sim.zoom} zoomControl={false} className="h-full w-full z-0" style={{ background: '#0B1520', cursor: pickingActive ? 'crosshair' : undefined }}>
            ...
            {activeFilters.vehicles && (
              <>
                {sim.vehicles.map(v => <SmoothMarker key={v.id} vehicle={v} routePoints={sim.roadNetworks[v.route] || sim.roadNetworks['CBD_Loop']} />)}
                {sim.policeVehicles.map(v => <SmoothMarker key={v.id} vehicle={v} routePoints={sim.roadNetworks[v.route] || sim.roadNetworks['Inner_Ring']} />)}
              </>
            )}

            {activeFilters.roadblocks && roadblocksList.map(rb => (
              <div key={rb.id}>
                <Marker position={[rb.lat, rb.lng]} icon={roadblockIcon}><Popup><div className="text-xs"><p className="font-bold text-slate-900 mb-1">{rb.name}</p><p className="text-slate-600">{rb.officers} officers on site</p></div></Popup></Marker>
            ...
            {pickedLocation && (
              <Marker position={pickedLocation} icon={pinIcon}>
                <Popup><div className="text-xs"><p className="font-bold text-slate-900">New roadblock</p><p className="text-slate-500">{pickedLocation[0].toFixed(5)}, {pickedLocation[1].toFixed(5)}</p></div></Popup>
              </Marker>
            )}
          </MapContainer>
          ...
          {pickingActive && (
            <div className="absolute top-12 left-1/2 -translate-x-1/2 z-[2000] bg-blue-600 text-white px-4 py-2 rounded-xl shadow-2xl flex items-center gap-3 animate-pulse">
              <svg viewBox="0 0 24 24" className="w-4 h-4 stroke-white fill-none" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              <span className="text-xs font-bold">Click on the map to place the roadblock</span>
              <button onClick={cancelPicking} className="ml-2 px-2 py-1 bg-white/20 rounded-lg text-[10px] font-bold hover:bg-white/30 transition-colors">Cancel</button>
            </div>
          )}
          ...
          {pickFeedback === 'success' && (
            <div className="absolute top-24 left-1/2 -translate-x-1/2 z-[2000] bg-emerald-600 text-white px-4 py-2 rounded-xl shadow-2xl flex items-center gap-2">
              <svg viewBox="0 0 24 24" className="w-4 h-4 stroke-white fill-none" strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg>
          ...
          <div className="absolute top-3 right-3 z-[1000] flex gap-1.5 flex-wrap">
            {['violations','expiredDiscs','roadblocks','hotspots','vehicles'].map(key => {
              const f = { key, label: key==='violations'?'Violations':key==='expiredDiscs'?'Expired':key==='roadblocks'?'Roadblocks':key==='hotspots'?'Hotspots':'Vehicles', color: key==='expiredDiscs'?'amber':key==='roadblocks'?'blue':key==='vehicles'?'emerald':'red' };
              const { bg, border, text } = FILTER_COLORS[f.color] || FILTER_COLORS.red;
              return (
                <button key={f.key} onClick={() => toggleFilter(f.key)}
                  className={`px-3 py-1.5 rounded-full text-[10px] font-medium border transition-colors cursor-pointer ${activeFilters[f.key] ? `${bg} ${border} ${text}` : 'bg-adm/90 border-slate-700 text-slate-500 opacity-50 hover:opacity-75'}`}>
        ...
        <div className="hidden lg:flex w-60 bg-adm2 border-l border-slate-800 flex-col shrink-0">
          ...
          <div className="p-2 border-t border-slate-800">
            <button onClick={startPicking} disabled={pickingActive}
              className={`w-full py-2 rounded-md text-[11px] font-semibold transition-colors ${pickingActive ? 'bg-blue-900 text-blue-300 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-500'}`}>
