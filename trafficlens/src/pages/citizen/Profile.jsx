      ...
      >
        <div className="bg-gradient-to-r from-ca to-ca-dark p-5 text-white">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
              {success ? 'Profile Updated' : 'Edit Profile'}
            </h2>
            {!isSaving && (
        ...
        {success ? (
          ...
          <div className="p-8 flex flex-col items-center gap-4">
            ...
            <p className="text-sm font-semibold text-slate-900">Saving changes...</p>
          </div>
        ) : (
      ...
      <div className="flex flex-col gap-4">
        ...
        {saveMessage && (
          <motion.div
            ...
            className="bg-slate-800 text-white text-xs px-4 py-2 rounded-lg text-center capitalize"
          >
            {saveMessage}
          </motion.div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1.4fr] gap-3.5">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4">
            ...
            <div className="space-y-0">
              {[
                ...
                { label: 'Date of birth', value: new Date(userData.date_of_birth).toLocaleDateString('en-ZA', { day: 'numeric', month: 'long', year: 'numeric' }) },
                { label: 'ID Number', value: userData.id_number },
                ...
                <div key={i} className={`flex justify-between items-center py-1.5 ${i < 5 ? 'border-b border-slate-200' : ''}`}>
                  ...
                  <span className="text-xs font-medium text-slate-900 text-right max-w-[60%] truncate">{item.value}</span>
                </div>
              ))}
            </div>
            ...
            <button 
              ...
              className="w-full mt-3 py-2 bg-ca text-white border-none rounded text-xs font-medium hover:bg-ca-dark transition-colors"
            >
              Edit profile
            </button>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-3.5">
            ...
            {[
              ...
              <div 
                key={item.key} 
                className={`flex items-start justify-between py-2 gap-2.5 ${index < arr.length - 1 ? 'border-b border-slate-200' : ''}`}
              >
                ...
                <button
                  ...
                  className={`w-[34px] h-[19px] rounded-full relative shrink-0 mt-0.5 transition-all duration-200 ${
                    consents[item.key] ? 'bg-emerald-500' : 'bg-slate-300'
                  } ${item.required ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer hover:opacity-80'}`}
                >
                  <div 
                    className={`w-[15px] h-[15px] rounded-full bg-white absolute top-0.5 shadow-sm transition-all duration-200 ${
                      consents[item.key] ? 'left-[17px]' : 'left-0.5'
                    }`} 
                  />
                </button>
              </div>
            ...
            <div className="mt-3 p-2 bg-slate-50 rounded-lg text-[10px] text-slate-500 text-center">
              {Object.entries(consents).filter(([key]) => key !== 'identity' && key !== 'vehicle' && key !== 'payment').map(([key, value]) => (
                <span key={key} className="mr-3 last:mr-0">
                  {key.replace(/_/g, ' ')}: <span className={value ? 'text-emerald-600 font-medium' : 'text-slate-400'}>
                    {value ? 'ON' : 'OFF'}
                  </span>
                </span>
              ))}
