...
const documentIcons = {
  license: { bg: '#E8F3FB', color: '#1B6CA8', icon: <><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></> },
  registration: { bg: '#ECFDF5', color: '#10B981', icon: <><rect x="1" y="9" width="22" height="11" rx="2"/><path d="M5 9V6a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v3"/><circle cx="7" cy="17" r="1.5"/><circle cx="17" cy="17" r="1.5"/></> },
  residence: { bg: '#EFF6FF', color: '#3B82F6', icon: <><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></> },
  receipt: { bg: '#FFFBEB', color: '#F59E0B', icon: <><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></> }
};
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-[#94A3B8] mt-0.5">{documents.length} documents on file</p>
          </div>
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-[10px] overflow-hidden">
          {documents.map((doc, index) => {
            ...
            return (
              <div key={doc.id} className={`flex items-center gap-3 px-4 py-3 ${index < documents.length - 1 ? 'border-b border-[#E2E8F0]' : ''}`}>
                <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: iconConfig.bg }}>
                ...
                <span className={`px-2.5 py-1 rounded-full text-[10px] font-semibold ${doc.status === 'valid' ? 'bg-[#ECFDF5] text-[#059669]' : 'bg-[#E8F3FB] text-[#1B6CA8]'}`}>
