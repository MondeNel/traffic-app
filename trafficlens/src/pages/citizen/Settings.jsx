      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-[1fr_1.4fr] gap-3.5">
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-[10px] p-4">
            <div className="text-xs font-medium text-[#0F172A] mb-3">
              Account information
            </div>
            ...
            <div className="space-y-0">
              <div className="flex justify-between items-center py-2 border-b border-[#E2E8F0]">
                ...
                <span className="text-xs font-medium text-[#0F172A]">{demoData.user.email}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-[#E2E8F0]">
                <label className="text-[11px] text-[#94A3B8]">Phone</label>
                <span className="text-xs font-medium text-[#0F172A]">{demoData.user.phone}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-[#E2E8F0]">
                ...
                <select 
                  ...
                  className="text-xs font-medium text-[#0F172A] border border-[#E2E8F0] rounded px-2 py-1 bg-white"
                >
              ...
              <div className="flex justify-between items-center py-2">
                ...
                <button className="text-xs font-medium text-[#1B6CA8] hover:text-[#0F4A7A]">
                  Change password
                </button>
              </div>
            </div>
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-[10px] p-4">
            ...
            {[
              ...
              <div 
                key={item.key}
                className={`flex items-center justify-between py-2.5 ${index < arr.length - 1 ? 'border-b border-[#E2E8F0]' : ''}`}
              >
                <div>
                ...
                <button
                  ...
                  className={`w-[34px] h-[19px] rounded-full relative flex-shrink-0 transition-colors cursor-pointer ${
                    settings[item.key] ? 'bg-[#1B6CA8]' : 'bg-[#E2E8F0]'
                  }`}
                >
                  <div 
                    className={`w-[15px] h-[15px] rounded-full bg-white absolute top-0.5 transition-all ${
                      settings[item.key] ? 'left-[17px]' : 'left-0.5'
                    }`}
                  />
                </button>
              </div>
        ...
        <div className="bg-white border border-[#FECACA] rounded-[10px] p-4">
          ...
          <button className="px-3 py-1.5 bg-white border border-[#FECACA] text-[#EF4444] rounded text-xs font-medium hover:bg-[#FEF2F2] transition-colors">
            Delete account
          </button>
        </div>
      </div>
