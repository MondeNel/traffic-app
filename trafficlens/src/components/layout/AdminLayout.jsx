      ...
      <div className="bg-adm2 border-b border-slate-800 px-4 py-3 flex items-center justify-between shrink-0 z-20">
        <div className="flex items-center gap-2">
          ...
          <div className="md:hidden">
            <h1 className="text-sm font-semibold text-slate-200 truncate max-w-[180px]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
              TrafficLens Admin
            </h1>
          </div>
        </div>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden fixed top-12 left-0 bottom-0 w-56 bg-adm2 border-r border-slate-800 z-50 overflow-y-auto">
                ...
                <div key={idx}>
                  ...
                  {group.items.map((item) => (
                    <button
                      ...
                      className={`flex items-center gap-2 px-2 py-2.5 rounded-md cursor-pointer text-[11px] mb-0.5 transition-all w-full text-left ${
                        isActive(item.path) 
            ...
            <div className="p-3 border-t border-slate-800">
              ...
              <button 
                onClick={handleLogout}
                className="flex items-center gap-2 px-2 py-2 rounded-md cursor-pointer text-[11px] w-full text-left text-red-400 hover:bg-red-500/10"
              >
                <svg viewBox="0 0 24 24" className="w-4 h-4 stroke-current fill-none" strokeWidth="1.5">
                  <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/>
                </svg>
        ...
        {mobileNavItems.map((item) => {
          ...
          return (
            <button
              key={item.path}
              onClick={() => navigate(item.path)}
              className={`flex flex-col items-center gap-0.5 py-1 px-1.5 rounded-lg min-w-[48px] transition-colors ${
